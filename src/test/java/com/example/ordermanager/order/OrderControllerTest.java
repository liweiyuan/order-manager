...
class OrderControllerTest {
  ...
  @Autowired private ObjectMapper objectMapper; // To convert objects to JSON
  ...
  @MockBean private OrderEventPublisher orderEventPublisher; // Mock the new event publisher
  ...
  void createOrderShouldReturnBadRequestWhenInventoryItemNotFound() throws Exception {
    ...
    OrderRequest orderRequest = new OrderRequest(1L, Collections.singletonList(lineItemRequest));
  ...
  void createOrderShouldCreateOrderAndPublishEvent() throws Exception {
    ...
    InventoryItem inventoryItem = new InventoryItem("Item1", "Description for Item1", 10);
    ...
    verify(orderEventPublisher, times(1))
        .publishOrderCreated(eq(10L), anyList()); // Verify new publisher
  }
  ...
  void updateOrderStatusShouldUpdateStatusWhenOrderFound() throws Exception {
    ...
    verify(orderRepository, times(1)).save(any(Order.class));
    verify(orderEventPublisher, times(1))
        .publishOrderStatusUpdated(eq(10L), eq("PENDING"), eq("SHIPPED"));
  ...
  void updateOrderStatusShouldPublishDeliveredEvent() throws Exception {
    ...
    verify(orderRepository, times(1)).save(any(Order.class));
    verify(orderEventPublisher, times(1))
        .publishOrderStatusUpdated(eq(10L), eq("SHIPPED"), eq("DELIVERED"));
  ...
  void updateOrderStatusShouldPublishCancelledEvent() throws Exception {
    ...
    verify(orderRepository, times(1)).save(any(Order.class));
    verify(orderEventPublisher, times(1))
        .publishOrderStatusUpdated(eq(10L), eq("PENDING"), eq("CANCELLED"));
    verify(orderEventPublisher, times(1))
        .publishOrderCancelled(eq(10L), eq("Order status updated to cancelled"));
  }
